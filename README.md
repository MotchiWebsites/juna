...
- Python 3.14 (pinned in `.python-version` for Vercel and CI)
- Node.js 24 (pinned in `.nvmrc` for local development and CI)
- npm 10 or newer
...
4. Create your local environment file:
    ...
    To use the Neon CLI, link this checkout to the shared project. This creates
    a local, ignored `.neon` context file:
...
| `npm run dev:django`               | Run only the Django development server on `127.0.0.1:8000` |
...
| `DJANGO_SECURE_HSTS_SECONDS`            | No         | Production HSTS lifetime; defaults to `31536000` for preload readiness                |
| `DJANGO_SECURE_HSTS_INCLUDE_SUBDOMAINS` | No         | Includes subdomains in the production HTTPS-only policy; defaults to `True`           |
| `DJANGO_SECURE_HSTS_PRELOAD`            | No         | Sends the HSTS preload directive in production; defaults to `True`                    |
...
`npm run dev:django` uses the local HTTP settings module, and CI plus deploy
paths use the HTTPS-safe production settings module. Vercel preview hostnames
and CSRF origins are handled by `config/production_settings.py`, and
preview responses receive `X-Robots-Tag: noindex, nofollow`. Production
...
secure cookies, HTTPS redirects, and a preload-ready HSTS policy by default.
...
5. Verify the production and redirect hosts over HTTPS, then submit the
   canonical host to the HSTS preload list when ready.
